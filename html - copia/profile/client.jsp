...
<sql:query var="resultcuenta" dataSource="jdbc/p">
                                                ...
                                                <div>
                                                    <table class="table table-sm table-striped table-hover">
														...
														<c:forEach var="row" items="${result.rows}">
															<tr>
																<td>
																	<a href="#" name="cliente" class="link link--style-1"><strong>${row.cedula}</strong></a>
																</td>
																<td>${row.nombre_cliente} ${row.apellido_cliente}</td>
																<td><c:forEach var="contacto" items="${resultcuenta.rows}">
																		${contacto.atributo_contacto}
																	</c:forEach>
																</td>
																<td>
                                                                    <a href="" type="submit" name="login" var="" class="btn btn-styled btn-base-1 btn-outline">Cotizar</a>
                                                                </td>
															</tr>
														</c:forEach>
															<tr>																
																<td>
																	<a href="#" class="btn btn-styled btn-block btn-rounded btn-base-1">Registrar Cliente</a>
																</td>
															</tr>
                                                        </tbody>
                                                    </table>
													<c:if test="${param.login != null}">
														<sql:query var="result2" dataSource="jdbc/p">
															select count(*) as kount from CLIENTE where CEDULA = ? 
														...
														<c:forEach var="row" items="${result2.rows}">
															<c:choose>
																<c:when test="${row.kount > 0}">
																	...
																	<c:set scope="session" var="cl" value="${param.cliente}"/>
																	<c:redirect url="cotiza.jsp"/>
																</c:when>
																<c:otherwise>
																	<c:set scope="session" value="Usuario y/o contaseña invalidos" var="cl"/>
																</c:otherwise>
															</c:choose>
														</c:forEach>
													</c:if>
